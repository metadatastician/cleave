...
The `src/` and `tests/` trees currently hold generic RSR-template scaffolding
(an FFI skeleton differing from the old groove repo's only by symbol prefix);
they test the template, not cleave, and are scheduled for deletion — see
`PRUNE-MANIFEST.md` (layout standard: `groove-protocol/docs/REPO-LAYOUT.adoc`,
ADR 0008).

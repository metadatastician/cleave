...
cookbook:
    ...
    echo "= cleave Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/cleave.1 << EOF
    .TH cleave 1 "$(date +%Y-%m-%d)" "{{version}}" "cleave Manual"
    .SH NAME
    cleave \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/cleave.1"
...
container-init:
    ...
    read -rp "Service name (e.g. my-api) [cleave]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-cleave}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t cleave:latest -f container/Containerfile .
    elif [ -f "build/Containerfile" ]; then
        podman build -t cleave:latest -f build/Containerfile .
    elif [ -f "Containerfile" ]; then
        podman build -t cleave:latest -f Containerfile .
    else
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push cleave:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it cleave:latest {{args}}

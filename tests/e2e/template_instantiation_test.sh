...
find "$TEST_REPO_PATH" -type f \
    ...
    -exec bash -c '
        file="$1"
        placeholder_pairs=(
            "cleave|$TEST_REPO_NAME"
            "hyperpolymath|$TEST_OWNER"
            "github.com|$TEST_FORGE"
            "CLEAVE|$TEST_PROJECT_NAME"
            "cleave|'"${TEST_REPO_NAME//-/_}"'"
            "Cleave is a transmutable interface-engine library: a dial from raw FFI-ABI through sandboxed (SNIF) to a unified Groove protocol, over a well-founded, self-dual, zero-residue teardown backbone.|$TEST_DESCRIPTION"

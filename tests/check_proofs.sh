...
mkdir -p "$1"
proof_run="$(mktemp -d "$1/cleave-proofs.XXXXXX")"
compiler="${IDRIS2:-idris2}"
"$compiler" --version
"$compiler" --build-dir "$proof_run/package" --typecheck cleave-proofs.ipkg
"$compiler" --build-dir "$proof_run/positive" --typecheck tests/proofs/positive.ipkg
for entry in reject-duplication:linear reject-leak:linear; do
  fixture="${entry%%:*}"
  diagnostic="${entry#*:}"

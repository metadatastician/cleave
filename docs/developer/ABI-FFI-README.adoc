...
│       └── include/
│           └── cleave.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cleave.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cleave.h
```
...
int main() {
    void* handle = cleave_init();
    if (!handle) return 1;

    int result = cleave_process(handle, 42);
    if (result != 0) {
        const char* err = cleave_last_error();
...
Compile with:
```bash
gcc -o example example.c -lcleave -L./zig-out/lib
```

### From Idris2

```idris
import CLEAVE.ABI.Foreign
...
#[link(name = "cleave")]
extern "C" {
    fn cleave_init() -> *mut std::ffi::c_void;
    fn cleave_free(handle: *mut std::ffi::c_void);
    fn cleave_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cleave_init();
        assert!(!handle.is_null());

        let result = cleave_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcleave = "libcleave"

function init()
    handle = ccall((:cleave_init, libcleave), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cleave_process, libcleave), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cleave_free, libcleave), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/cleave.h
   ```
